...
The mismatch occurs when the HTML component relies on browser-native behaviors (like object-fit: cover or CSS transforms) that @react-pdf does not natively support in the same way. To fix this, we must abstract the _logic_ of the component from its _rendering_.

**The "Universal Component" Strategy:**

We define a shared set of props that describe the _state_ of the component, not its implementation.
...
3. **Separation of Concerns:** Store _User Preferences_ (theme, last opened file ID) in localStorage (fast, small). Store _Document Content_ in the file system or IndexedDB (slow, large).
