...
export function LookAheadSection({ config, reportData }: Props) {
  const items = reportData.lookAhead || [];
          ...
          <tbody className="divide-y divide-zinc-100">
            {items.map((item: any, i: number) => (
              <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-zinc-50/50'}>
                <td className="px-4 py-2 text-zinc-400 font-mono text-xs">{item.id || i + 1}</td>
                <td className="px-4 py-2 font-medium text-zinc-900">{item.task}</td>
                <td className="px-4 py-2 text-center text-zinc-600 font-mono text-xs">{item.startDate}</td>
                <td className="px-4 py-2 text-center text-zinc-600 font-mono text-xs">{item.endDate}</td>
                <td className="px-4 py-2">
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-zinc-700 w-8 text-right">{item.progress}%</span>
                   </div>
                </td>
              </tr>
            ))}
